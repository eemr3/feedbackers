...
describe('Home', () => {
	...
	it('should render create account modal when click on header account create account button', () => {
		cy.visit(APP_URL)
	...
	it('should show on error with on ivalid email', () => {
		cy.visit(APP_URL)
	...
	it('should show on error with on ivalid email', () => {
		...
		cy.get('#email-field').type('igor@igor.me')
		cy.get('#password-field').type('2')
		cy.get('#submit-button').click()

		cy.get('#password-error')
	})

	it('should logout work correclty', () => {
		cy.visit(APP_URL)
